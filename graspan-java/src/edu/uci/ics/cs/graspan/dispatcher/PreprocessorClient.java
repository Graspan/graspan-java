...
public class PreprocessorClient {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 * for scanning main.config and pp.pgen.config
		 */
		...
		while ((ln = preprocessorConfigStream.readLine()) != null) {
			tok = ln.split(" ");
//			if (tok[0].compareTo("GRAPH_PATH") == 0) {
...
//			}
			...
			if (tok[0].compareTo("HAS_EDGE_VALS") == 0) {
				GlobalParams.setHasEdgeVals(tok[2].trim());
			}
...
//			}
			...
			if (tok[0].compareTo("1ST_VTX_ID") == 0) {
...
//			GraspanTimer ppERedgeAdding = new GraspanTimer(System.currentTimeMillis());
    		...
    		GraphERuleEdgeAdderAndSorter edgeAdder = new GraphERuleEdgeAdderAndSorter();
			edgeAdder.run();
...
//			logger.info("Edge Adding from Erules took: "+ GraspanTimer.getDurationInHMS(ppERedgeAdding.getDuration()));
			...
			logger.info("sizeOfGraphWithEREdgs(MB)"+gSize);
			
			double pSize_expected = getExpectedPartSize_MB();
			logger.info("getExpectedPartSize_MB " + pSize_expected);
...
//		memUsage.stop();
	...
	private static void printValstoCompconfFile() throws IOException {
		...
		compconfStrm = new PrintWriter(new BufferedWriter(new FileWriter("comp.config", false)));
		
		compconfStrm.println("<START_CONFIG_FILE_BODY> (AUTOGENERATED)");
	...
	private static void printValstoPgenconfFile(int numparts_torequest, double pSize_postNewEdges_inNumOfEdges) throws IOException {
		...
		pgenconfStrm = new PrintWriter(new BufferedWriter(new FileWriter("pp.pgen.config", false)));
		
		pgenconfStrm.println("<START_CONFIG_FILE_BODY> (AUTOGENERATED)");
		...
		pgenconfStrm.println("HAS_EDGE_VALS = " + GlobalParams.hasEdgeVals());
