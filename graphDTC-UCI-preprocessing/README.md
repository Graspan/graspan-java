...
It is a very good idea to study the example applications carefully. There are currently three example applications in the package **gDTCpreproc.apps**:
...
```
    ...
    pagerank = LOAD '/user/akyrola/graphs/soc-LiveJournal1.txt' USING gDTCpreproc.apps.pig.PigPagerank as (vertex:int, rank:float);
